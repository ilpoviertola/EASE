...
class VGGish(VGG):
    def __init__(
        ...
        self.dense = dense
        if self.dense:
            assert spec_mean is not None and spec_std is not None

        if pretrained:
            if vggish_ckpt_path is not None:
                state_dict = torch.load(vggish_ckpt_path, map_location="cpu")
    ...
    def _preprocess(self, x, fs):
        ...
        if self.dense:
            x = (x - self.spec_mean) / self.spec_std
        return x
