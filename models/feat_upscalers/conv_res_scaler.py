...
def make_encoder(in_dim, kernel_size, qk_dim, num_layers=2):
...
def create_coordinate(h, w, start=0, end=1, device="cuda", dtype=torch.float32):
    ...
    coord_map = torch.stack([xx, yy], axis=-1)[None, ...]  # type: ignore
    coords = rearrange(coord_map, "b h w c -> b (h w) c", h=h, w=w)
    return coords


class ConvResScaler(nn.Module):
    def __init__(
        self,
        qk_dim: int = 128,
        v_dim: int = 320,
        ...
        num_heads: int = 4,
        num_groups: int = 32,
        encode_guide_image: bool = True,
        image_proj: nn.Module = nn.Identity(),
        feature_proj: nn.Module = nn.Identity(),
    ):
        super().__init__()
        self.qk_dim = qk_dim
        self.input_dim = input_dim
        self.kernel_size = kernel_size
        self.encode = encode_guide_image
        self.image_proj = image_proj
        self.feature_proj = feature_proj

        self.key_encoder = make_encoder(qk_dim, 1, qk_dim)
        self.query_encoder = make_encoder(qk_dim, 1, qk_dim)
        self.norm = nn.GroupNorm(
            num_groups=num_groups, num_channels=qk_dim, affine=False
        )

        self.key_features_encoder = make_encoder(v_dim, 1, qk_dim)
        self.sft_key = SFTModulation(qk_dim, qk_dim)
        self.cross_decode = CrossAttentionBlock(qk_dim, qk_dim, v_dim, num_heads)

        if encode_guide_image:
            self.guide_image_encoder = make_encoder(input_dim, kernel_size, qk_dim)
    ...
    def encode_guide_image(self, image):
        if not self.encode:
            return image

        # Extract high-level features of image.
        encoded_image = self.guide_image_encoder(image)
    ...
    def forward(
        ...
        encoded_image = self.encode_guide_image(image)
        encoded_image = rearrange(encoded_image, "b c h w -> b h w c")
        encoded_image = self.image_proj(encoded_image)
        encoded_image = rearrange(encoded_image, "b h w c -> b c h w")
