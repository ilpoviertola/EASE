...
class MaskClassificationS4(LightningModule):
    def __init__(
        self,
        network: nn.Module,
    ...
    def _select_logits(self, mask_logits_per_block, class_logits_per_block):
        """For training data of S4 we only have GTs for every 5th mask and class logits.

        Args:
            mask_logits_per_block (list[torch.Tensor]): Mask logits.
            class_logits_per_block (list[torch.Tensor]): Class logits.

        Returns:
            tuple: A tuple containing the selected mask and class logits.
        """
        if self.training:
            BT = mask_logits_per_block[0].shape[0]
            indices = torch.tensor(
                list(range(0, BT, 5)), device=mask_logits_per_block[0].device
            )
            _mask_logits_per_block, _class_logits_per_block = [], []
            for mask_logits, class_logits in zip(
                mask_logits_per_block, class_logits_per_block
            ):
                mask_logits = torch.index_select(mask_logits, dim=0, index=indices)
                class_logits = torch.index_select(class_logits, dim=0, index=indices)
                _mask_logits_per_block.append(mask_logits)
                _class_logits_per_block.append(class_logits)
            return _mask_logits_per_block, _class_logits_per_block
    ...
    def eval_step(
        ...
        for i, (mask_logits, class_logits) in enumerate(
            ...
            if self._using_vpo and not self.per_frame:
                logits = logits[0::5]
                targets = targets[0::5]
                is_neg_samples = is_neg_samples[0::5]
            ...
            if self.save_masks_on_eval and i == len(mask_logits_per_layer) - 1:
                # flatten list of list[str] to list[str]
                uids = []
                for t in _targets:
                    uids += t["uids"]
    ...
    @property
    def _using_vpo(self) -> bool:
        if hasattr(self.trainer.test_dataloaders, "dataset"):
            return self.trainer.test_dataloaders.dataset.__class__.__name__.startswith(
                "VPO"
            )
        return False
