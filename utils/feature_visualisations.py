...
def write_video_with_audio(
    video_frames, audio_array, video_fps, audio_fps, output_path
):
    """
    ...
    video_options = {"crf": "23", "preset": "slow", "bit_rate": "1000k"}
    ...
    if audio_array is not None:
        ...
        wavfile.write(
            temp_audio_path,
            audio_fps,
            audio_array.cpu().to(torch.float64).permute(1, 0).numpy(),
        )
...
def alpha_blend_layers(layers):
    ...
    for layer in layers[1:]:
        ...
        rgb_out = (rgb2 * alpha2 + rgb1 * alpha1 * (1 - alpha2)) / alpha_out.clamp(
            min=1e-7
        )
        blended_image = torch.cat([rgb_out, alpha_out], dim=1)
    return (
        (blended_image[:, :3] * 255).clamp(0, 255).to(torch.uint8).permute(0, 2, 3, 1)
    )
    ...
    with torch.no_grad():
        ...
        for frame_num in range(n_frames):
            ...
            for head in range(sim_by_head.shape[1]):
                selected_sim = F.interpolate(
                    sim_by_head[frame_num, head, :, :, selected_audio_feat]
                    .unsqueeze(0)
                    .unsqueeze(0),
                    size=(vh, vw),
                    mode="bicubic",
...
red_cmap = mcolors.LinearSegmentedColormap(
    "RedMap",
    segmentdata={
...
blue_cmap = mcolors.LinearSegmentedColormap(
    "BlueMap",
    segmentdata={
...
def plot_attention_video(
    sims_by_head, frames, audio, video_fps, audio_fps, output_filename
):
...
def plot_2head_attention_video(
    sims_by_head, frames, audio, video_fps, audio_fps, output_filename
):
    prepped_sims = _prep_sims_for_plotting(sims_by_head, frames)
    ...
    layer1 = torch.cat([frames, torch.ones(n_frames, 1, vh, vw)], axis=1)
    layer2_head1 = torch.tensor(red_cmap(sims_1.squeeze().detach().cpu())).permute(
        0, 3, 1, 2
    )
    layer2_head2 = torch.tensor(blue_cmap(sims_2.squeeze().detach().cpu())).permute(
        0, 3, 1, 2
    )
...
def plot_feature_video(
    ...
    with torch.no_grad():
        ...
        red_audio_feats = F.interpolate(
            red_audio_feats, size=(50, red_img_feats.shape[0]), mode="bicubic"
        )
    ...
    red_audio_feats_expanded = F.interpolate(
        red_audio_feats_expanded, scale_factor=6, mode="bicubic"
    )
    ...
    write_video_with_audio(
        (red_audio_feats_expanded.permute(0, 2, 3, 1) * 255)
        .clamp(0, 255)
        .to(torch.uint8),
        audio,
        video_fps,
...
def plot_fused_feature_image(
    feats,
    frames,
    fused_feats=None,
    save_name_prefix="fused_feat",
    save_frames=False,
):
    if fused_feats is not None:
        with torch.no_grad():
            fused_feats_ = fused_feats.cpu().to(torch.float32)
            feats_ = feats.cpu().to(torch.float32)
            [fused_feats_, feats_], _ = pca([fused_feats_, feats_])
            _, _, vh, vw = frames.shape
            fused_feats = F.interpolate(fused_feats_, size=(vh, vw), mode="bicubic")
            feats = F.interpolate(feats_, size=(vh, vw), mode="bicubic")
            for i in range(feats.shape[0]):
                img = (fused_feats[i].permute(1, 2, 0).numpy() * 255).astype(np.uint8)
                img_pil = torchvision.transforms.ToPILImage()(img)
                img_pil.save(f"{save_name_prefix}_{i:03d}.png")
                img = (feats[i].permute(1, 2, 0).numpy() * 255).astype(np.uint8)
                img_pil = torchvision.transforms.ToPILImage()(img)
                img_pil.save(f"orig_feat_{i:03d}.png")
    else:
        with torch.no_grad():
            feats_ = feats.cpu().to(torch.float32)
            [feats_], _ = pca([feats_])
        ...
        for i in range(feats.shape[0]):
            ...
            if save_frames:
                ...
                img_pil = torchvision.transforms.ToPILImage()(img)
                img_pil.save(f"orig_img_{i:03d}.png")
